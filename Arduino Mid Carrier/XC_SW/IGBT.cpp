...
void init_igbt_pwm() {
  ...
  // 3) Configure TIM3 master (center-aligned)
  {
    ...
    if (status != HAL_OK) { Serial.print("[IGBT] Error: TIM3 HAL_TIM_PWM_Init failed! Status: "); Serial.println(status); return; }
    ...
    if (status != HAL_OK) { Serial.print("[IGBT] Error: TIM3 HAL_TIMEx_MasterConfigSynchronization failed! Status: "); Serial.println(status); return; }
  ...
  // 4) Configure TIM1 slave (center-aligned)
  {
    ...
    if (status != HAL_OK) { Serial.print("[IGBT] Error: TIM1 HAL_TIM_PWM_Init failed! Status: "); Serial.println(status); return; }
    ...
    if (status != HAL_OK) { Serial.print("[IGBT] Error: TIM1 HAL_TIMEx_ConfigBreakDeadTime failed! Status: "); Serial.println(status); return; }
    ...
    if (status != HAL_OK) { Serial.print("[IGBT] Error: TIM1 HAL_TIM_SlaveConfigSynchro failed! Status: "); Serial.println(status); return; }
  ...
  if (status != HAL_OK) { Serial.print("[IGBT] Error: TIM3 HAL_TIM_PWM_ConfigChannel CH2 failed! Status: "); Serial.println(status); return; }
  ...
  status = HAL_TIM_PWM_ConfigChannel(&htim1, &oc, TIM_CHANNEL_2);
   if (status != HAL_OK) { Serial.print("[IGBT] Error: TIM1 HAL_TIM_PWM_ConfigChannel CH2 failed! Status: "); Serial.println(status); return; }
  ...
  if (status != HAL_OK) { Serial.print("[IGBT] Error: TIM1 HAL_TIM_PWM_Start CH2 failed! Status: "); Serial.println(status); return; }
  Serial.println("[IGBT] TIM1 CH2 PWM Started.");

  status = HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_2); // Start Master Channel
  if (status != HAL_OK) { Serial.print("[IGBT] Error: TIM3 HAL_TIM_PWM_Start CH2 failed! Status: "); Serial.println(status); HAL_TIM_PWM_Stop(&htim1, TIM_CHANNEL_2); return; }
...
void update_igbt_pwm() {
  ...
  bool allowOut = !PowerState::faultLockout && hwEnable;
  ...
  bool pwmAllowed = !PowerState::faultLockout;
  ...
  snprintf(debugLine, sizeof(debugLine),
    ...
    PowerState::faultLockout, hwEnable, allowOut,
